...
class MultiVAETrainingPlan(AdversarialTrainingPlan):
    """Custom TrainingPlan that incorporates sutom losses: integration, cycle and modality reconstruction losses."""

    def validation_step(
        self, batch: torch.Tensor, batch_idx  # TODO check if can get rid of
        ...
        :param batch:
            Batch of data
        :returns:
            Dictionary with averaged losses and n_obs in the batch
        """
    ...
    def training_step(
        self, batch: torch.Tensor, batch_idx, optimizer_idx: int = 0
    ) -> Dict[str, Union[torch.Tensor, int]]:
    ...
    def training_epoch_end_mil(self, outputs):
        """Log custom losses after each epoch."""
        n_obs, elbo, rec_loss, kl_local, integ = 0, 0, 0, 0, 0
        n_modalities = 0
        modality_losses = []
        for key in outputs[0].keys():
